...
module.exports =
  DocBlockrAtom = (function() {
    ...
    DocBlockrAtom.prototype.wrap_lines_command = function() {
      ...
      var indent_spaces_same_tag = this.repeat(' ', (this.editor_settings.indentation_spaces_same_tag ? this.editor_settings.indentation_spaces_same_tag : num_indent_spaces));
      ...
      var wrap_para = function(para) {
        ...
        for(_i = 0; _len = words.length, _i < _len; _i++) {
          ...
          if((line.length + word.length) > wrap_len) {
            ...
            text+= line.replace(/\s+$/, '') + '\n';
            if (para_tagged) {
                line = line_prefix + indent_spaces_same_tag + word + ' ';
            } else {
      ...
      for(i = 0; len = wrapped_paras.length, i < len; i++) {
        ...
        var _tag, next_is_tagged;
        if(i == len - 1) {
          _tag = next_is_tagged = false;
        }
        else {
          _tag = wrapped_paras[i+1].tag;
          next_is_tagged = wrapped_paras[i+1].tagged;
        }
        next_is_same_tag = ((next_is_tagged && para.tag) == _tag);

        text += para.text;
        if (!last && !para.line_tagged && !next_is_tagged) {
            text += '\n' + line_prefix;
        } else if (!spacer_between_sections || next_is_same_tag) {
            // do nothing
        } else {
          text += '\n' + line_prefix;
        }
